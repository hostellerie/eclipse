...
{
    $path = isset($_SERVER['PHP_SELF']) ? str_replace('\\', '/', strtolower($_SERVER['PHP_SELF'])) : '';
...
function theme_css_eclipse()
{
    ...
    $version = '?v=1.0.0-rc53';
    $requestPath = isset($_SERVER['PHP_SELF']) ? str_replace('\\', '/', strtolower($_SERVER['PHP_SELF'])) : '';
    $isAdmin = strpos($requestPath, '/admin/') !== false;
    $isAdminDashboard = $isAdmin && substr($requestPath, -10) === '/index.php';
    $isStoryEditor = $isAdmin && substr($requestPath, -10) === '/story.php';
    $isCommentPage = !$isAdmin && (substr($requestPath, -12) === '/article.php' || substr($requestPath, -12) === '/comment.php');

    $cssFiles = array(
        array('name' => 'denim-base', 'file' => '/layout/' . $_CONF['theme_default'] . '/css_' . $LANG_DIRECTION . '/style.css', 'attributes' => array('media' => 'all'), 'priority' => 100),
        array('name' => 'eclipse-variables', 'file' => '/layout/' . $_CONF['theme'] . '/css/variables.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 200),
        array('name' => 'eclipse-base', 'file' => '/layout/' . $_CONF['theme'] . '/css/base.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 210),
        array('name' => 'eclipse-layout', 'file' => '/layout/' . $_CONF['theme'] . '/css/layout.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 220),
        array('name' => 'eclipse-components', 'file' => '/layout/' . $_CONF['theme'] . '/css/components.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 230),
        array('name' => 'eclipse-forms', 'file' => '/layout/' . $_CONF['theme'] . '/css/forms.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 240),
        array('name' => 'eclipse-plugins', 'file' => '/layout/' . $_CONF['theme'] . '/css/plugins.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 250),
        array('name' => 'eclipse-responsive', 'file' => '/layout/' . $_CONF['theme'] . '/css/responsive.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 260),
        array('name' => 'eclipse-modern', 'file' => '/layout/' . $_CONF['theme'] . '/css/modern.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 270),
        array('name' => 'eclipse-ui-fixes', 'file' => '/layout/' . $_CONF['theme'] . '/css/ui-fixes.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 290),
        array('name' => 'eclipse-v3', 'file' => '/layout/' . $_CONF['theme'] . '/css/v3.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 300),
        array('name' => 'eclipse-footer-links', 'file' => '/layout/' . $_CONF['theme'] . '/css/footer-links.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 305),
    );
    if ($isAdminDashboard) {
        $cssFiles[] = array('name' => 'eclipse-studio', 'file' => '/layout/' . $_CONF['theme'] . '/css/studio.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 280);
    }
    if ($isStoryEditor) {
        $cssFiles[] = array('name' => 'eclipse-story-editor', 'file' => '/layout/' . $_CONF['theme'] . '/css/story-editor.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 310);
    }
    if ($isAdmin && substr($requestPath, -18) === '/configuration.php') {
        $cssFiles[] = array('name' => 'eclipse-configuration', 'file' => '/layout/' . $_CONF['theme'] . '/css/configuration.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 310);
    }
    if ($isCommentPage) {
        $cssFiles[] = array('name' => 'eclipse-comments', 'file' => '/layout/' . $_CONF['theme'] . '/css/comments.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 310);
    }
    if (eclipse_menu_plugin_active()) {
        $cssFiles[] = array('name' => 'eclipse-menu', 'file' => '/layout/' . $_CONF['theme'] . '/css/menu.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 315);
        $cssFiles[] = array('name' => 'eclipse-menu-refinements', 'file' => '/layout/' . $_CONF['theme'] . '/css/menu-refinements.css' . $version, 'attributes' => array('media' => 'all'), 'priority' => 316);
    }
    return $cssFiles;
}
...
{
    global $_CONF;
    return array(array('file' => '/layout/' . $_CONF['theme'] . '/js/theme.js?v=1.0.0-rc53', 'footer' => true, 'priority' => 100));
}
...
function eclipse_theme_options()
{
    ...
    $defaults = array(
        ...
        'color_scheme' => 'light', 'mobile_menu' => true, 'editor_hide_sidebars' => true, 'share_facebook' => false, 'share_linkedin' => false, 'share_x' => false,
    ...
    if (!empty($_CONF['path_data'])) {
        $savedFile = rtrim($_CONF['path_data'], '/\\') . DIRECTORY_SEPARATOR . 'eclipse-settings.json';
        if (is_file($savedFile) && is_readable($savedFile)) {
            $saved = json_decode((string) @file_get_contents($savedFile), true);
            if (is_array($saved)) {
                $options = array_merge($options, eclipse_sanitize_options($saved));
            }
        }
    }
    return $options;
...
function eclipse_sanitize_options($input)
{
    ...
    $choices = array(
        ...
        'color_scheme' => array('light', 'dark', 'auto'),
    );
...
function eclipse_data_json($name, $fallback = array())
{
    ...
    $path = $dataDir === '' ? '' : $dataDir . DIRECTORY_SEPARATOR . $name;
    if ($path === '' || !is_file($path) || !is_readable($path)) return $fallback;
...
function eclipse_write_data_json($name, $value)
{
    ...
    return $dataDir !== '' && is_dir($dataDir) && is_writable($dataDir) && $json !== false && @file_put_contents($dataDir . DIRECTORY_SEPARATOR . $name, $json . "\n", LOCK_EX) !== false;
}
...
function eclipse_footer_editor($data)
{
    ...
    $html = '<fieldset data-studio-section="footer-links" class="eclipse-footer-editor"><legend>Footer links</legend><p class="eclipse-section-intro">Create up to eight link rows. Data is stored in <code>data/eclipse-footer.json</code> and survives theme updates.</p><div class="eclipse-footer-groups">';
...
function eclipse_render_customizer()
{
    ...
    if (isset($_POST['eclipse_update'])) {
        ...
        if (!function_exists('SEC_checkToken') || !SEC_checkToken()) {
            ...
            $dataDir = !empty($_CONF['path_data']) ? rtrim($_CONF['path_data'], '/\\') : '';
            $backupRoot = $dataDir . DIRECTORY_SEPARATOR . 'eclipse-backups';
            ...
            if (!$validName) {
                ...
                $message = '<p class="eclipse-notice eclipse-success">Theme backup restored. Reload the page and clear caches only if older assets remain visible.</p>';
            }
        }
        ...
        if (!function_exists('SEC_checkToken') || !SEC_checkToken()) {
            ...
            foreach ($history as $entry) {
                if (isset($entry['id'], $entry['settings']) && (string) $entry['id'] === $restoreId && is_array($entry['settings'])) {
                    eclipse_record_settings_history('Before history restore');
                    $restored = eclipse_write_data_json('eclipse-settings.json', eclipse_sanitize_options($entry['settings']));
                    break;
                }
            }
            $message = $restored ? '<p class="eclipse-notice eclipse-success">Settings restored from history.</p>' : '<p class="eclipse-notice eclipse-error">The selected history entry could not be restored.</p>';
        }
        ...
        if (!function_exists('SEC_checkToken') || !SEC_checkToken()) {
            ...
            if ($name === '') {
                ...
                unset($palettes[$name]);
                $message = eclipse_write_data_json('eclipse-palettes.json', $palettes) ? '<p class="eclipse-notice eclipse-success">Custom palette removed.</p>' : '<p class="eclipse-notice eclipse-error">Unable to update eclipse-palettes.json.</p>';
            } else {
                ...
                if (count($palette) !== 6) {
                    ...
                    ksort($palettes);
                    $message = eclipse_write_data_json('eclipse-palettes.json', $palettes) ? '<p class="eclipse-notice eclipse-success">Custom palette saved.</p>' : '<p class="eclipse-notice eclipse-error">Unable to write eclipse-palettes.json.</p>';
                }
            }
        }
        ...
        if (!function_exists('SEC_checkToken') || !SEC_checkToken()) {
            ...
            $dataDir = !empty($_CONF['path_data']) ? rtrim($_CONF['path_data'], '/\\') : '';
            $target = $dataDir . DIRECTORY_SEPARATOR . 'eclipse-settings.json';
            $footerTarget = $dataDir . DIRECTORY_SEPARATOR . 'eclipse-footer.json';
            eclipse_record_settings_history('Before restoring defaults');
            $settingsRemoved = !is_file($target) || @unlink($target);
            $footerRemoved = !is_file($footerTarget) || @unlink($footerTarget);
            if ($settingsRemoved && $footerRemoved) {
                $message = '<p class="eclipse-notice eclipse-success">Saved settings and footer links removed. Eclipse defaults will be used on the next page load.</p>';
            } else {
                $message = '<p class="eclipse-notice eclipse-error">Unable to remove one or more Eclipse data files.</p>';
            }
        } else {
            $submitted = isset($_POST['eclipse']) && is_array($_POST['eclipse']) ? $_POST['eclipse'] : array();
            ...
            eclipse_record_settings_history('Before saving settings');
            $settingsSaved = $dataDir !== '' && is_dir($dataDir) && is_writable($dataDir) && $json !== false && @file_put_contents($target, $json . "\n", LOCK_EX) !== false;
            $footerSaved = eclipse_write_data_json('eclipse-footer.json', $footerSettings);
            if ($settingsSaved && $footerSaved) {
                $message = '<p class="eclipse-notice eclipse-success">Eclipse settings and footer links saved in the data directory.</p>';
            } else {
                $message = '<p class="eclipse-notice eclipse-error">Unable to write one or more Eclipse data files. Check path_data and its permissions.</p>';
            }
        }
    }
    ...
    $themeBackups = array();
    $backupRoot = !empty($_CONF['path_data']) ? rtrim($_CONF['path_data'], '/\\') . DIRECTORY_SEPARATOR . 'eclipse-backups' : '';
    ...
    $environmentDataWritable = !empty($_CONF['path_data']) && is_dir($_CONF['path_data']) && is_writable($_CONF['path_data']);
    $environmentHtml = '<dl class="eclipse-environment"><div><dt>Geeklog</dt><dd>' . $h($environmentGeeklog) . '</dd></div><div><dt>PHP</dt><dd>' . $h(PHP_VERSION) . '</dd></div><div><dt>ZIP updates</dt><dd class="' . (class_exists('ZipArchive') ? 'is-ok' : 'is-warning') . '">' . (class_exists('ZipArchive') ? 'Available' : 'ZipArchive missing') . '</dd></div><div><dt>Persistent data</dt><dd class="' . ($environmentDataWritable ? 'is-ok' : 'is-warning') . '">' . ($environmentDataWritable ? 'Writable' : 'Not writable') . '</dd></div></dl>';
    ...
    $html .= '</div></fieldset>' . eclipse_footer_editor($footerData) . '<fieldset class="eclipse-portability"><legend>Portability and history</legend><div class="eclipse-portability-grid"><div><h3>Import or export</h3><p>Import a JSON draft, review it in Preview, then save it.</p><input type="file" id="eclipse-settings-import" accept="application/json,.json"><button type="button" id="eclipse-settings-export">Export current settings</button></div><div><h3>Settings history</h3><label><span>Saved snapshot</span><select name="eclipse_history_id"><option value="">Select a snapshot</option>';
...
function eclipse_install_uploaded_update($upload)
{
    ...
    $backupRoot = $dataDir . DIRECTORY_SEPARATOR . 'eclipse-backups';
    if (!is_dir($backupRoot) && !@mkdir($backupRoot, 0750, true)) { eclipse_remove_tree($job); return $fail('Unable to create data/eclipse-backups.'); }
    ...
    if (!eclipse_copy_tree($sourceTheme, $themeDir, 0755, 0644)) { eclipse_remove_tree($job); return $fail('The update copy failed. Restore the latest folder from data/eclipse-backups.'); }
    eclipse_remove_tree($job);
    $cacheMessage = ' Geeklog cache could not be cleared automatically because CTL_clearCache() is unavailable.';
    if (function_exists('CTL_clearCache')) {
        CTL_clearCache();
        $cacheMessage = ' The Geeklog template and generated CSS caches were cleared automatically.';
    }
